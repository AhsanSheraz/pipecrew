...
Scans existing repos, generates `platform.md`, per-repo `agent-context/` and `CLAUDE.md`, per-repo `DESIGN_SYSTEM.md` for frontends, workspace-specific agents (`{slug}-product-owner`, `{slug}-assessor`, `{slug}-troubleshooter`; the UX consultant uses the base `pipecrew:ux-consultant`). Teams has no onboarding flow.
